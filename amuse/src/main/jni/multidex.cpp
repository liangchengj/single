...
// Modified by Liangcheng Juves 2020/6/6 8:49
//
#include <jni.h>
#include <zlib.h>
#include <unistd.h>
...
#define LOG_TAG "MULTIDEX.NATIVE"
...
extern "C"
{
    ...
    using func_dvmRawDexFileOpen = int (*)(const char *fileName, const char *odexOutputName,
                                           void *ppRawDexFile, bool isBootstrap);
...
#define CHECK_EXCEPTION_AND_ABORT(MSG) CHECK_EXCEPTION_AND_EXE_ABORT(MSG, )
                                                     ...
                                                     const char *signature = "([B)I")
    {
        ...
        {
            if ((strncmp(name, func->name, len_name) == 0) && (strncmp(signature, func->signature, len_name) == 0))
            {
    ...
    Java_com_meyoustu_amuse_multidex_Native_obtainCheckSum(JNIEnv *env, jclass, jstring filePath)
    {
    ...
    Java_com_meyoustu_amuse_multidex_Native_loadDirectDex(JNIEnv *env, jclass,
                                                          jstring jFilePath,
                                                          jbyteArray jFileContents)
    {
        ...
        else
        {
            ...
            if (jFileContents == nullptr)
            {
                ...
                array_object_ptr = static_cast<ArrayObject *>(
                    malloc(sizeof(ArrayObject) - sizeof(ArrayObject::contents) + length));
            ...
            else
            {
                ...
                uint8_t *jarray_ptr = static_cast<uint8_t *>(
                    env->GetPrimitiveArrayCritical(jFileContents, nullptr));
        ...
        env->SetObjectField(dex_file, sGuardField,
                            env->CallStaticObjectMethod(sCloseGuardClazz,
                                                        sGuardGetMethod)); // set guard

        return dex_file;
    }

    JNIEXPORT void JNICALL
    Java_com_meyoustu_amuse_multidex_Native_recoverAction(JNIEnv *, jclass)
    {
        if (sIsSetHandler)
        {
            sigaction(SIGSEGV, &OldSignalAction, nullptr);
    ...
    Java_com_meyoustu_amuse_multidex_Native_makeOptDexFile(JNIEnv *env, jclass,
                                                           jstring jFilePath,
    ...
    JNIEXPORT jboolean JNICALL
    Java_com_meyoustu_amuse_multidex_Native_initialize(JNIEnv *env, jclass, jint sdkVersion,
                                                       jclass runtimeExceptionClass)
    {
        jclass clazz = env->FindClass("dalvik/system/DexFile");
        CHECK_EXCEPTION;
        sDexFileClazz = static_cast<jclass>(env->NewGlobalRef(clazz));
        CHECK_EXCEPTION;
        sCookieField = env->GetFieldID(sDexFileClazz, "mCookie", "I");
        CHECK_EXCEPTION;
        sFileNameField = env->GetFieldID(sDexFileClazz, "mFileName", "Ljava/lang/String;");
        CHECK_EXCEPTION;
        sGuardField = env->GetFieldID(sDexFileClazz, "guard", "Ldalvik/system/CloseGuard;");
        CHECK_EXCEPTION;

        clazz = env->FindClass("dalvik/system/CloseGuard");
        CHECK_EXCEPTION;
        sCloseGuardClazz = static_cast<jclass>(env->NewGlobalRef(clazz));
        CHECK_EXCEPTION;

        sGuardGetMethod = env->GetStaticMethodID(sCloseGuardClazz, "get",
        ...
        dvmRawDexFileOpen = (func_dvmRawDexFileOpen)dlsym(handler,
        ...
        {
            // SDK = 19
            clazz = env->FindClass("com/android/dex/Dex");
            CHECK_EXCEPTION;
            sDexClazz = static_cast<jclass>(env->NewGlobalRef(clazz));
            CHECK_EXCEPTION;
            sDexConstructor = env->GetMethodID(sDexClazz, "<init>", "([B)V");
            CHECK_EXCEPTION;
    ...
    JNIEXPORT JNICALL jint JNI_OnLoad(JavaVM *vm, void *)
    {
        ...
        jclass clazz = env->FindClass("com/meyoustu/amuse/multidex/Native");

        // speed up first invocation of native methods
        static JNINativeMethod native_methods[] = {
            {"obtainCheckSum",
             "(Ljava/lang/String;)J",
             (void *)Java_com_meyoustu_amuse_multidex_Native_obtainCheckSum},
        };
