...
class DexInstallProcessor {
  ...
  void doInstallation(final Context mainContext, File sourceApk, Result result) throws Exception {
    ...
    Utility.clearDirFiles(
        new File(filesDir.getParent(), Constants.CODE_CACHE_SECONDARY_FOLDER_NAME));
  ...
  void doInstallationInOptProcess(Context context, File apkFile) throws Exception {
    ...
    while ((dexEntry =
            apkZipFile.getEntry(Constants.DEX_PREFIX + secondaryNumber + Constants.DEX_SUFFIX))
        != null) {
  ...
  private void installSecondaryDexes(ClassLoader loader, List<DexHolder> dexHolderList)
      throws Exception {
  ...
  private List<DexHolder> obtainDexObjectList(
      File apkFile, File rootDir, File dexDir, File odexDir, File zipDir, Result result)
      throws IOException {
    ...
    boolean isModified =
        (mPreferences.getLong(keyApkTime, Constants.NO_VALUE) != archiveTimeStamp)
    ...
    if (isModified) {
      ...
      while ((dexEntry =
        ...
        if (Native.isSupportFastLoad()) {
          ...
          if (Utility.isBetterUseApkBuf()) {
            ...
            dexHolderList.add(
                DexHolder.obtainValidDexBuffer(
                    mPreferences, secondaryNumber, validDexFile, optDexFile));
          }
        } else {
          // all dex or zip
          if (Environment.getDataDirectory().getFreeSpace() > Constants.SPACE_THRESHOLD) {
            dexHolderList.add(
                DexHolder.obtainValidForceDexOpt(
                    mPreferences, secondaryNumber, dexFile, optDexFile, apkZipFile, dexEntry));
          } else {
            ...
            dexHolderList.add(
                DexHolder.obtainValidZipDex(
                    mPreferences, secondaryNumber, zipFile, zipOptFile, apkZipFile, dexEntry));
          }
        }
        secondaryNumber++;
      }
      ...
      for (int secondaryNumber = 2; secondaryNumber <= totalDexNum; secondaryNumber++) {
        dexHolderList.add(obtainDexHolder(secondaryNumber, apkFile, dexDir, odexDir, zipDir));
      }
    }
  ...
  private DexHolder obtainDexHolder(
      int secondaryNumber, File apkFile, File dexDir, File odexDir, File zipDir)
      throws IOException {
    int type =
        mPreferences.getInt(
            Constants.KEY_DEX_OBJ_TYPE + secondaryNumber, Constants.LOAD_TYPE_INVALID);
    if (type == Constants.LOAD_TYPE_INVALID) {
      if (Native.isSupportFastLoad()) {
        type =
            Utility.isBetterUseApkBuf() ? Constants.LOAD_TYPE_APK_BUF : Constants.LOAD_TYPE_DEX_BUF;
      } else {
    ...
    if (type == Constants.LOAD_TYPE_ZIP_OPT) {
      ...
      if (isZipFileValid(zipFile, secondaryNumber)) {
        ...
        ZipEntry dexFileEntry =
            apkZipFile.getEntry(Constants.DEX_PREFIX + secondaryNumber + Constants.DEX_SUFFIX);
        DexHolder.ZipOpt zipOpt =
            DexHolder.obtainValidZipDex(
                mPreferences, secondaryNumber, zipFile, zipOptFile, apkZipFile, dexFileEntry);
        apkZipFile.close();
        return zipOpt;
      }
    }
    ...
    if (type == Constants.LOAD_TYPE_DEX_OPT) {
      ...
      if (validDexFile != null) {
        ...
        if (Native.isSupportFastLoad()) {
          ...
          ZipEntry dexFileEntry =
              apkZipFile.getEntry(Constants.DEX_PREFIX + secondaryNumber + Constants.DEX_SUFFIX);
          return DexHolder.obtainValidForceDexOpt(
              mPreferences, secondaryNumber, dexFile, optDexFile, apkZipFile, dexFileEntry);
        }
      }
    }
  ...
  private byte[] obtainEntryBytesInApk(ZipFile apkZipFile, ZipEntry dexFileEntry)
      throws IOException {
    return Utility.obtainEntryBytesInZip(apkZipFile, dexFileEntry);
  }

  private File obtainEntryFileInApk(ZipFile apkZipFile, ZipEntry dexFileEntry, File target)
      throws IOException {
  ...
  private File getValidDexFile(File file, int secondaryNumber) throws IOException {
    if (!checkFileValid(
        secondaryNumber, Constants.KEY_DEX_CHECKSUM, Constants.KEY_DEX_TIME, file, false)) {
      return null;
    }
  ...
  private File getValidOptDexFile(File file, int secondaryNumber) throws IOException {
    ...
    if (!checkFileValid(
        secondaryNumber, Constants.KEY_ODEX_CHECKSUM, Constants.KEY_ODEX_TIME, file, false)) {
      return null;
    }
  ...
  private boolean checkFileValid(
      int secondaryNumber, String keyCheckSum, String keyTime, File file, boolean isZip) {
    if (!file.exists()) {
    ...
    if (expectedModTime != lastModified) {
      Monitor.get()
          .logWarning(
              "Invalid file: "
                  + " (key \""
                  + (keyCheckSum + keyTime + secondaryNumber)
                  + "\"),"
    ...
    if (doCheckSum) {
      long expectedCheckSum =
          mPreferences.getLong(keyCheckSum + secondaryNumber, Constants.NO_VALUE);
  ...
  private boolean isZipFileValid(File zipFile, int secondaryNumber) {
    return checkFileValid(
        secondaryNumber, Constants.KEY_DEX_CHECKSUM, Constants.KEY_DEX_TIME, zipFile, true);
  }
}
