...
class DexInstallProcessor {
    ...
    void doInstallation(final Context mainContext, File sourceApk, Result result)
            throws Exception {
    ...
    void doInstallationInOptProcess(Context context, File apkFile)
            throws Exception {
            ...
            while ((dexEntry = apkZipFile.getEntry(Constants.DEX_PREFIX + secondaryNumber +
                ...
                if (Native.isSupportFastLoad()) {
                    ...
                    if (Utility.isBetterUseApkBuf()) {
                        byte[] bytes = obtainEntryBytesInApk(apkZipFile, dexEntry);
                        dexHolderList.add(new DexHolder.ApkBuffer(secondaryNumber, bytes, dexFile,
                                optDexFile));
                    } else {
                        File validDexFile = obtainEntryFileInApk(apkZipFile, dexEntry, dexFile);
                        dexHolderList.add(DexHolder.obtainValidDexBuffer(mPreferences, secondaryNumber,
                                validDexFile, optDexFile));
                    }
                } else {
                    // all dex or zip
                    if (Environment.getDataDirectory().getFreeSpace() > Constants.SPACE_THRESHOLD) {
                        dexHolderList.add(DexHolder.obtainValidForceDexOpt(mPreferences, secondaryNumber,
                                dexFile, optDexFile, apkZipFile, dexEntry));
                    } else {
                        ...
                        dexHolderList.add(DexHolder.obtainValidZipDex(mPreferences, secondaryNumber,
                                zipFile, zipOptFile, apkZipFile, dexEntry));
                    }
                }
                secondaryNumber++;
            }
    ...
    private DexHolder obtainDexHolder(int secondaryNumber, File apkFile, File dexDir,
                                      File odexDir, File zipDir)
            throws IOException {
        int type = mPreferences.getInt(Constants.KEY_DEX_OBJ_TYPE + secondaryNumber,
        ...
        if (type == Constants.LOAD_TYPE_DEX_OPT) {
            ...
            if (validDexFile != null) {
                ...
                if (validOptDexFile != null) {
                    return new DexHolder.DexOpt(secondaryNumber, validDexFile,
                            validOptDexFile, false);
                } else {
                    if (Native.isSupportFastLoad()) {
    ...
    private File getValidDexFile(File file, int secondaryNumber)
            throws IOException {
    ...
    private File getValidOptDexFile(File file, int secondaryNumber)
            throws IOException {
        if (!file.exists()) {
