...
public class OptimizeService extends IntentService {
  ...
  public void onCreate() {
    ...
    try {
      ...
      mRootDir = Utility.ensureDirCreated(filesDir, Constants.multidex_DIR_NAME);
  ...
  private void handleOptimize() throws IOException {
    ...
    try {
      ...
      SharedPreferences preferences =
          this.getSharedPreferences(Constants.PREFS_FILE, Context.MODE_PRIVATE);
      int totalDexNum = preferences.getInt(keyApkDexNum, 0);
      for (int secondaryNumber = 2; secondaryNumber <= totalDexNum; secondaryNumber++) {
        int type =
            preferences.getInt(
                Constants.KEY_DEX_OBJ_TYPE + secondaryNumber, Constants.LOAD_TYPE_APK_BUF);
        ...
        Monitor.get()
            .logInfo("Process beginning holder " + dexHolder.toString() + ", type: " + type);
        ...
        while (fasterHolder != null) {
          ...
          try {
            ...
            if (fasterHolder != null) {
              ...
              Monitor.get()
                  .reportAfterInstall(cost, freeSpace, reducedSpace, fasterHolder.toString());
            }
          } catch (Throwable tr) {
