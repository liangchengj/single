...
abstract class DexHolder {
  ...
  private static void putZipOptInfo(
      SharedPreferences.Editor editor, int secondaryNumber, File zipFile) throws IOException {
  ...
  private static void putDexFileInfo(
      SharedPreferences.Editor editor, int secondaryNumber, File file) throws IOException {
    String keyCheckSum = Constants.KEY_DEX_CHECKSUM;
  ...
  private static void putDexOptInfo(
      SharedPreferences.Editor editor, int secondaryNumber, File optFile) throws IOException {
  ...
  static DexHolder obtainValidDexBuffer(
      SharedPreferences preferences, int secondaryNumber, File validDexFile, File optDexFile)
      throws IOException {
  ...
  static DexHolder obtainValidForceDexOpt(
  ...
  static DexHolder obtainValidDexOpt(
      SharedPreferences preferences, int secondaryNumber, File validDexFile, File optDexFile)
      throws IOException {
    SharedPreferences.Editor editor = preferences.edit();
    putTypeInfo(editor, secondaryNumber, Constants.LOAD_TYPE_DEX_OPT);
  ...
  static DexHolder.ZipOpt obtainValidZipDex(
      SharedPreferences preferences,
      int secondaryNumber,
      File validZipFile,
      File validZipOptFile,
      ZipFile apkZipFile,
      ZipEntry dexFileEntry)
      throws IOException {
  ...
  static class ApkBuffer extends DexHolder {
    ...
    public Object toDexFile() {
      try {
        return Native.loadDirectDex(null, mBytes);
      } catch (Exception e) {
        Monitor.get().logError("Fail to create DexFile: " + toString(), e);
        Result.get().unFatalThrowable.add(e);
        return null;
